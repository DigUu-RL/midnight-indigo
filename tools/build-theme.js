...
/*
 * Emits icons/theme/midnight-indigo-icon-theme.json. Called by build-icons.js
 * with the icon names it just generated, so a mapping can never point at an
 * icon that does not exist — the build fails instead.
 */

const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, '..', 'icons', 'theme', 'midnight-indigo-icon-theme.json');

const key = (kind, name) => `_${kind}_${name.replace(/-/g, '_')}`;

const F = (name) => key('file', name);
const D = (name) => key('folder', name);
...
module.exports = function buildTheme({ files, folders }) {
  const fileSet = new Set(files);
  const folderSet = new Set(folders);

  const iconDefinitions = {};
  for (const name of files) iconDefinitions[F(name)] = { iconPath: `../svg/file-${name}.svg` };
  iconDefinitions._folder = { iconPath: '../svg/folder.svg' };
  iconDefinitions._folder_open = { iconPath: '../svg/folder-open.svg' };
  for (const name of folders) {
    iconDefinitions[D(name)] = { iconPath: `../svg/folder-${name}.svg` };
    iconDefinitions[`${D(name)}_open`] = { iconPath: `../svg/folder-${name}-open.svg` };
  }

  const mapFiles = (table, what) => {
    const out = {};
  ...
  console.log(`wrote icons/theme/midnight-indigo-icon-theme.json (${Object.keys(iconDefinitions).length} definitions)`);
};
