...
const TMP = path.join(require('os').tmpdir(), 'midnight-indigo-preview');

const theme = require(path.join(ROOT, 'themes', 'midnight-indigo-color-theme.json'));
...
/* The file tree shown in the hero, as [depth, icon, label, state]. */
const TREE = [
...
function withGutter(html, lineCount) {
...
function cardPage(html, item, lineCount) {
...
function heroPage(html, lineCount) {
...
function shoot(browser, htmlFile, pngFile, width, height) {
  execFileSync(
    browser,
    [
      '--headless',
...
async function main() {
  ...
  const injections = ['jsx-components', 'csharp-delegates'].map((file) => {
    const grammar = require(path.join(ROOT, 'injections', `${file}.tmLanguage.json`));
    const scopes = grammar.injectionSelector.split(',').map((s) => s.trim().replace(/^L:/, ''));
...
function writeGallery(items) {
  ...
  const heading = (label) => `## ${label.replace(/#/g, '\\#')}`;
...
function iconFor(ext) {
  const map = require(path.join(ROOT, 'icons', 'theme', 'midnight-indigo-icon-theme.json'));
