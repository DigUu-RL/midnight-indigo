...
/*
 * Measures the geometry build-icons.js cannot know on its own, and writes it to
 ...
 *
 * build-icons.js turns these into a translate for every glyph and an x/y for
...
const { FONT, WEIGHT, textRuns, textKey } = require('./icon-spec');

const HTML = path.join(os.tmpdir(), 'midnight-indigo-measure.html');
const GLYPH_OUT = path.join(__dirname, 'glyph-bounds.json');
const TEXT_OUT = path.join(__dirname, 'text-bounds.json');
...
const flat = (fn) => fn('#fff', '#000', WHITE);
...
const offBy = (b) => Math.max(Math.abs(b.cx), Math.abs(b.cy));
